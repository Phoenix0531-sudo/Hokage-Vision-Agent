...
- The repository originally contained a large amount of upstream-like YOLOv5 source code in the root tree, including `detect.py`, `train.py`, `val.py`, `export.py`, `hubconf.py`, `models/`, `utils/`, `classify/`, `segment/`, and `data/`. This legacy tree was moved under `legacy/old_project/` and has since been removed from the working tree; git history preserves the full snapshot for audit purposes.
...
- Git history indicates a root GPL-3.0 `LICENSE` existed in the first commit and was later deleted. The legacy `setup.cfg` lived under `legacy/old_project/` (now removed from the working tree; retrievable from git history).
- Project-specific legacy files included `base_ui.py`, `main_window.py`, `main_window.ui`, `test_open_image.py`, `datasets/classes.txt`, and readme/demo images under `legacy/old_project/` (now removed from the working tree; retrievable from git history).
- The legacy tree included label files under `legacy/old_project/datasets/labels/` and sample images under `legacy/old_project/data/images/`, but no source manifest or redistribution statement. That tree is removed from the working tree (git history retains it); the files must not be redistributed as an official dataset.
...
- Legacy YOLOv5 code: governed by upstream YOLOv5 licensing. The previously isolated `legacy/old_project/` tree has been removed from the working tree; git history preserves it for audit purposes.
...
- Legal status and redistribution permission for any Naruto/Hokage dataset images used to produce the label files that lived under `legacy/old_project/datasets/labels/` (removed from working tree, git history). Current project policy is no redistribution and no commercial use.
- Legal status of the readme/demo images that lived under `legacy/old_project/readme_images*` (removed from working tree, git history).
